...
/* From Windows 10 1607+, 24 bytes in 64 bit windows, 16 in 32 bit*/
typedef struct{
    volatile LARGE_INTEGER phase;
    SRWLOCK lock;
    LONG max_threads;
    LONG max_spins;
...
WINBASEAPI BOOL        WINAPI DeleteSynchronizationBarrier(LPSYNCHRONIZATION_BARRIER);
...
WINBASEAPI BOOL        WINAPI EnterSynchronizationBarrier(LPSYNCHRONIZATION_BARRIER, DWORD); 
...
WINBASEAPI BOOL        WINAPI InitializeSynchronizationBarrier(LPSYNCHRONIZATION_BARRIER, LONG, LONG);
